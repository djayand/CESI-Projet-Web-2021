...
try {
    ...
    if (!$r) {
        ...
        if ($r->fetch()) {
            ...
            $q2 = "SELECT ID_Personne FROM personne WHERE Login = '$pseudo' AND Mdp = SHA1('$elsa7')";
            $r2 = $db->query($q2);

            if($row = $r2->fetch(PDO::FETCH_ASSOC)){
                //Affichage de la premiere colonne
                setcookie('ID', $row['ID_Personne'], time() + 7 * 24 * 3600, $path = "/", null, false, true);
            }

            // Cookie
            setcookie('pseudo', $pseudo, time() + 7 * 24 * 3600, $path = "/", null, false, true);
